...
#' project_mgmt
#'
#' @return Values used to set up excalidraw projects
#' @noRd
...
handle_new_project <- function(project){
  ...
  project_path <- file.path(app_sys("projects"), project_name, Sys.Date())
  ...
  excal <- file.path(project_path, sprintf("%s.excalidraw", project_name))
  return(excal)
}
# handle_new_project(project = "rr-2023")
# handle_new_project()
...
handle_open_project <- function(project) {
  ...
  if (missing(project)) {
    # Prompt for new project
    project_path <- excal_prompt_existing_project_name()
    excal <- file.path(project_path, sprintf("%s.excalidraw", project))
  } else {
    # The user has entered a project name
    # We need to make sure this project can
    # either be created, or opened
    ...
    # If a project is specified, check if it exists
    if (!dir.exists(app_sys("projects", project))) {
      ...
      project_path <- file.path(app_sys("projects"), project, Sys.Date())
      # project doesn't exist, we create it
      dir.create(project_path, recursive = TRUE)
    } else {
      # It does not exist, we return the path
      project_path <- file.path(app_sys("projects"), project, Sys.Date())
    }
  }
