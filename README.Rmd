...
When launched (unless specifying `with_project = FALSE`), the app will prompt either for a new project, or for opening an existing one.
If you develop in an existing project, `{shinidraw}` will load the latest version of the `.excalidraw` file.
Inside the project folder, the excalidraw file are stored in subfolders by date, so that you can go back to previous versions of your drawings.
